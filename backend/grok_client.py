...
# -*- coding: utf-8 -*-
"""
Grok API Client for Hebrew Text Processing
Extracts structured participant information from unstructured Hebrew text
"""
...
class GrokClient:
    """Client for interacting with Grok API to extract information from Hebrew text."""
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            logger.warning("No Grok API key provided. Set GROK_API_KEY environment variable or pass api_key parameter.")
    
    def extract_participant_info(self, hebrew_text: str) -> List[Dict[str, str]]:
        """
        Extract participant information from Hebrew text using Grok API.
        ...
        """
        if not self.api_key:
            logger.error("No Grok API key available")
            return []
    ...
    def identify_activity_type(self, hebrew_text: str) -> Optional[str]:
        """
        Identify the activity type from Hebrew text using Grok API.
...
JSON Response:"""
    ...
    def _parse_extraction_response(self, response: str) -> List[Dict[str, str]]:
        """Parse the extraction response from Grok API."""
        try:
